...
func RepositoryCommit() (*github.RepositoryCommit, error) {
	var err error
	commitOnce.Do(func() {
		err = json.Unmarshal([]byte(commitText), &commit)
	})
	if err != nil {
		return nil, err
	}
...
func RepositoryCommits(length int) ([]*github.RepositoryCommit, error) {
	values := make([]*github.RepositoryCommit, length)
	for i := 0; i < length; i++ {
		v, err := RepositoryCommit()
		if err != nil {
			return nil, err
		}
		values[i] = v
